  ...
  return (
    <div className="space-y-10 animate-fade-in">
      <div>
        ...
        <p className="text-lg text-slate-500 mt-1">Real-time status of your Customer Success AI Agent</p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="bg-white p-8 rounded-2xl border border-slate-200 shadow-sm hover:shadow-md transition-all">
          <h2 className="text-xl font-bold text-slate-900 mb-8">Tickets by Channel</h2>
          <ChannelPieChart data={charts.by_channel} />
        </div>
        <div className="bg-white p-8 rounded-2xl border border-slate-200 shadow-sm hover:shadow-md transition-all">
          <h2 className="text-xl font-bold text-slate-900 mb-8">Status Distribution</h2>
          <StatusBarChart data={charts.by_status} />
        </div>
        <div className="bg-white p-8 rounded-2xl border border-slate-200 shadow-sm hover:shadow-md transition-all">
          <h2 className="text-xl font-bold text-slate-900 mb-8">Priority Level</h2>
      ...
      <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
        <div className="p-8 border-b border-slate-100 flex justify-between items-center bg-slate-50/30">
          ...
          <Link href="/admin/tickets" className="text-blue-600 text-base font-bold hover:text-blue-700 transition-colors flex items-center">
            View All <ExternalLink size={18} className="ml-2" />
          </Link>
        </div>
            ...
            <tbody className="divide-y divide-slate-100">
              {recent_tickets.map((ticket: any) => (
                <tr key={ticket.id} className="hover:bg-slate-50/80 transition-all group">
                  <td className="px-8 py-5 text-base font-mono text-slate-400">
                    {ticket.id.substring(0, 8)}...
                  </td>
                  <td className="px-8 py-5 text-base font-bold text-slate-700">
                  ...
                  <td className="px-8 py-5 text-base text-slate-500">
